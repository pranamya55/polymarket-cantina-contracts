...
/// @title NegRiskCtfCollateralAdapterEvents
/// @author Polymarket
/// @notice Events emitted by the NegRiskCtfCollateralAdapter contract.
abstract contract NegRiskCtfCollateralAdapterEvents {
    /// @notice Emitted when NO positions are converted into YES positions.
    /// @param initiator The address that initiated the conversion (msg.sender).
    /// @param marketId The neg risk market ID.
    /// @param indexSet Bitmask of question indices whose NO tokens were converted.
    /// @param amount The input amount of each NO position converted.
    /// @param amountOut The output amount of each YES position received (after fees).
    event PositionsConverted(
        address indexed initiator, bytes32 indexed marketId, uint256 indexed indexSet, uint256 amount, uint256 amountOut
    );
}
...
contract NegRiskCtfCollateralAdapter is CtfCollateralAdapter, NegRiskCtfCollateralAdapterEvents {
    ...
    function convertPositions(bytes32 _marketId, uint256 _indexSet, uint256 _amount) external onlyUnpaused(USDCE) {
        ...
        // Send YES tokens to caller
        {
            ...
            emit PositionsConverted(msg.sender, _marketId, _indexSet, _amount, amountOut);
        }

        // Wrap any received USDC.e into CollateralToken
        uint256 usdceAmount = ERC20(USDCE).balanceOf(address(this));
        if (usdceAmount > 0) {
