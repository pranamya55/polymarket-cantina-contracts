...
/// @title CtfCollateralAdapterEvents
/// @author Polymarket
/// @notice Events emitted by the CtfCollateralAdapter contract.
abstract contract CtfCollateralAdapterEvents {
    /// @notice Emitted when collateral is split into YES/NO positions.
    /// @param initiator The address that initiated the split (msg.sender).
    /// @param conditionId The condition ID that was split.
    /// @param amount The amount of collateral that was split.
    event PositionSplit(address indexed initiator, bytes32 indexed conditionId, uint256 amount);

    /// @notice Emitted when YES/NO positions are merged back into collateral.
    /// @param initiator The address that initiated the merge (msg.sender).
    /// @param conditionId The condition ID that was merged.
    /// @param amount The amount of each position that was merged.
    event PositionsMerged(address indexed initiator, bytes32 indexed conditionId, uint256 amount);

    /// @notice Emitted when positions are redeemed for collateral after resolution.
    /// @param initiator The address that initiated the redemption (msg.sender).
    /// @param conditionId The condition ID that was redeemed.
    /// @param amounts The amounts of each position that were redeemed.
    /// @param payout The total amount of collateral paid out to the initiator.
    event PositionsRedeemed(address indexed initiator, bytes32 indexed conditionId, uint256[] amounts, uint256 payout);
}
...
contract CtfCollateralAdapter is Pausable, ERC1155TokenReceiver, CtfCollateralAdapterEvents {
        ...
        onlyUnpaused(USDCE)
    {
        ...
        emit PositionsMerged(msg.sender, _conditionId, _amount);
    }
    ...
    function redeemPositions(address, bytes32, bytes32 _conditionId, uint256[] calldata) external onlyUnpaused(USDCE) {
        ...
        uint256 amount = ERC20(USDCE).balanceOf(address(this));
        ...
        emit PositionsRedeemed(msg.sender, _conditionId, amounts, amount);
    }
