...
/// @notice MarketIds are the keccak256 hash of the oracle, feeBips, and
/// metadata, with the final 8 bits set to 0
/// @notice QuestionIds share the first 31 bytes with their corresponding
/// MarketId, and the final byte consists of the
/// conditionIndex
...
library NegRiskIdLib {
    ...
    /// @notice Returns the QuestionId for a given MarketId and conditionIndex
    /// @param _marketId        - the marketId
    /// @param _conditionIndex  - the conditionIndex
    /// @return questionId      - the questionId
    function getQuestionId(bytes32 _marketId, uint8 _conditionIndex) internal pure returns (bytes32) {
        unchecked {
            return bytes32(uint256(_marketId) + _conditionIndex);
        }
    }

    /// @notice Returns the conditionIndex for a given QuestionId
    /// @param _questionId    - the questionId
    /// @return conditionIndex - the conditionIndex
    function getConditionIndex(bytes32 _questionId) internal pure returns (uint8) {
